...
module.exports = {
    ...
    addSubCategory: (req, res) => {
        ...
        let sql = `select * from category where name = '${subCategoryName}'`
        mysql_conn.query(sql, (err, results) => {
            if (err) {
                return res.status(500).send({ status: 'error', err })
            }
            ...
            upload(req, res, (err) => {
                ...
                mysql_conn.query(sql, (err, results) => {
                    ...
                    let dataDBProduct = {
                        ...
                        coverImage: productImage[0].filename,
                        popularCount: 0,
                    ...
                    mysql_conn.query(sql, dataDBProduct, (err, results) => {
                        ...
                        sql = `select id from product where coverImage = '${productImage[0].filename}'`;
                        mysql_conn.query(sql, (err, results) => {
                            if (err) {
                                return res.status(500).json({ message: "There's an error on the server. Please contact the administrator.", error: err });
                            }
